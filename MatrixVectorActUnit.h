...
#pragma HLS UNROLL
        ap_int<W_BIT> temp_wgt = weights( (p+1)*W_BIT-1, p*W_BIT);
        ap_int<IN_BIT> temp_in = in( (p+1)*IN_BIT-1, p*IN_BIT);
...
#pragma HLS LOOP_TRIPCOUNT min=16 max=16
#pragma HLS UNROLL
                out_buf( (p+1)*OUT_BIT-1, p*OUT_BIT) = ReLU<MUL_BIT, OUT_BIT, W_BIT, BIAS_BIT, RSHIFT>(acc[p], bias[p][out_fold_cnt], m0[p][out_fold_cnt]);
            }
#if MVAU_DEBUG
            cout << "$$$$$$$$$$$$$$$\nout_buf: " << endl;
...
#endif
            ap_int<W_BIT> temp_wgt = temp_wgt_simd( (p+1)*W_BIT-1, p*W_BIT);
            ap_int<IN_BIT> temp = temp_in( (p+1)*IN_BIT-1, p*IN_BIT);
            ap_int<W_BIT+IN_BIT> mul;
            mul = temp_wgt * temp;
            acc[index] += mul;
#if MVAU_DEBUG
...
#pragma HLS LOOP_TRIPCOUNT min=16 max=16
#pragma HLS UNROLL
                    out_buf((p + 1) * OUT_BIT - 1, p * OUT_BIT) = ReLU<MUL_BIT, OUT_BIT, W_BIT, BIAS_BIT, RSHIFT>(
...
#pragma HLS LOOP_TRIPCOUNT min=16 max=16
#pragma HLS UNROLL
                ap_int<IN_BIT> temp_addfm = addfm_buf((p+1)*OUT_BIT, p*OUT_BIT);
                out_buf( (p+1)*OUT_BIT-1, p*OUT_BIT) = ReLUAdd<MUL_BIT, OUT_BIT, W_BIT, BIAS_BIT, RSHIFT>(acc[p], temp_addfm, bias[p][out_fold_cnt], m0[p][out_fold_cnt], IS_ADD);
            }
#if MVAU_DEBUG
            cout << "$$$$$$$$$$$$$$$\nout_buf: " << endl;
            for (unsigned p = 0; p < PE; ++p) {
...
void DecvMatrixVectorActUnit(
    ...
    cout << "total_reps : " << total_reps << endl;
#endif
    ap_int<SIMD*IN_BIT> row_store[72]; //128/16*9
#pragma HLS RESOURCE variable=row_store core=RAM_2P_BRAM
