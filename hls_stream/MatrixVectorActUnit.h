...
#pragma HLS LOOP_TRIPCOUNT min=16 max=16
#pragma HLS UNROLL
        ap_int<27> temp_wgt_a = weights_a( (p<<LOG_W_BIT)+(W_BIT-1), (p<<LOG_W_BIT));
        temp_wgt_a = temp_wgt_a << 18;
        ap_int<27> temp_wgt_b = weights_b( (p<<LOG_W_BIT)+(W_BIT-1), (p<<LOG_W_BIT));
        ap_int<18> temp_in = in( (p<<LOG_IN_BIT)+(IN_BIT-1), (p<<LOG_IN_BIT));
        ap_int<45> mul;
        mul = (temp_wgt_a + temp_wgt_b) * temp_in;
        res_a += mul(33,18) + mul(15,15);
        res_b += mul(15,0);
    }
    res(20,0) = res_b;
    res(41,21) = res_a;
