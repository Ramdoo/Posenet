...
void LoadWgt1(wgt16_T* weight, wgt1_T wgt1[WGT_SIZE1][POSE_PE1], unsigned iter_block) {
...
void LoadWgt2(wgt16_T* weight, wgt2_T wgt2[WGT_SIZE2], unsigned iter_block) {
...
void LoadWgt3(wgt16_T* weight, wgt3_T wgt3[WGT_SIZE3][POSE_PE3], unsigned iter_block) {
...
void LoadBias1(bias8_T* bias, bias1_pe_T bias1[BIAS_M0_SIZE1], unsigned iter_block) {
...
void LoadBias2(bias8_T* bias, bias2_pe_T bias2[BIAS_M0_SIZE2], unsigned iter_block) {
...
void LoadBias3(bias8_T* bias, bias3_pe_T bias3[BIAS_M0_SIZE3], unsigned iter_block) {
...
void LoadM1(m16_T * m0, m0_1pe_T m0_1[BIAS_M0_SIZE1], unsigned iter_block) {
    for (int rep = 0; rep < config[iter_block].ic_nums2*48/POSE_PE1; rep=rep+2) {
        m16_T temp_m0 = m0[parm_size[iter_block].b1 + rep];
...
void LoadM2(m16_T * m0, m0_2pe_T m0_2[BIAS_M0_SIZE1], unsigned iter_block) {
    for (int rep = 0; rep < config[iter_block].ic_nums2*48/POSE_PE2; ++rep) {
        m0_2pe_T temp_m0;
        for (int pe = 0; pe < POSE_PE3/16; ++pe) {
            temp_m0((pe+1)*16*POSE_M0_BIT-1, pe*16*POSE_M0_BIT) =
                    m0[parm_size[iter_block].b1 + rep*POSE_PE2 + pe];
        }
        m0_2[rep] = temp_m0;
    }
}
...
void LoadM3(m16_T * m0, m0_3pe_T m0_3[BIAS_M0_SIZE1], unsigned iter_block) {
    for (int rep = 0; rep < config[iter_block].oc_nums3*16/POSE_PE3; rep=rep+2) {
        m16_T temp_m0 = m0[parm_size[iter_block].b3 + rep];
