...
#pragma HLS INTERFACE m_axi depth=497   port=m0     offset=direct bundle=m0
    ...
    ap_int<POSE_SIMD1*POSE_W_BIT>  wgt1[WGT_SIZE1][POSE_PE1];
    ap_int<POSE_SIMD2*POSE_W_BIT>  wgt2[WGT_SIZE2];
    ap_int<POSE_SIMD1*POSE_W_BIT>  wgt3[WGT_SIZE3][POSE_PE1];
    ap_int<POSE_BIAS_BIT> bias1[POSE_PE1][BIAS_M0_SIZE1];
    ap_int<POSE_BIAS_BIT> bias2[POSE_PE2][BIAS_M0_SIZE2];
    ap_int<POSE_BIAS_BIT> bias3[POSE_PE3][BIAS_M0_SIZE3];
    ap_uint<POSE_M0_BIT>  m0_1[POSE_PE1][BIAS_M0_SIZE1];
    ap_uint<POSE_M0_BIT>  m0_2[POSE_PE2][BIAS_M0_SIZE2];
    ap_uint<POSE_M0_BIT>  m0_3[POSE_PE3][BIAS_M0_SIZE3];
#pragma HLS ARRAY_PARTITION variable=wgt1 complete dim=2
#pragma HLS ARRAY_PARTITION variable=wgt3 complete dim=2
...
#pragma HLS ARRAY_PARTITION variable=m0_1 complete dim=1
#pragma HLS ARRAY_PARTITION variable=m0_2 complete dim=1
#pragma HLS ARRAY_PARTITION variable=m0_3 complete dim=1
...
#pragma HLS ALLOCATION instances=LoadM3      limit=1 function
    ...
    for (ap_uint<8> iter_block = 0; iter_block < BLOCK_NUMS; ++iter_block) {
        ...
        LoadWgt1(weight, wgt1, iter_block, true);
        LoadBias1(bias, bias1, iter_block, true);
        LoadM1(m0, m0_1, iter_block, true);
        LoadWgt2(weight, wgt2, iter_block, true);
        LoadBias2(bias, bias2, iter_block, true);
        LoadM2(m0, m0_2, iter_block, true);
        LoadWgt3(weight, wgt3, iter_block, true);
        LoadBias3(bias, bias3, iter_block, true);
        LoadM3(m0, m0_3, iter_block, true);
